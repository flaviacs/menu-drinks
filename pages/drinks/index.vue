...
<template>
  ...
  <div class="w-full md:w-9/12 m-auto text-center">
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 p-2">
      ...
      <div
        v-if="pending"
        v-for="post in [1, 2, 3, 4, 5]"
        class="h-20 bg-gradient-to-r from-slate-200 to-slate-300 rounded-md ease-in duration-100"
      ></div>

      <div v-else v-for="post in categories" class="inline-block align-middle">
        <router-link
          :to="
            '/drinks/' + post.strCategory?.toLowerCase()?.replace('/', '%2F')
          "
          class="rounded-lg text-white bg-gradient-to-r from-cyan-500 to-blue-500 block uppercase p-6 bg-sky-200 font-medium"
        >
          {{ post.strCategory }}
        </router-link>
      </div>
